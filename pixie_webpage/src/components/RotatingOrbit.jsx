...
export default function RotatingOrbit({}) {
    ...
    const updatePixels = (angle,t) => {
        ...
        let [a, b] = [400, 100]
        let phi = Math.PI / 6 - Math.sin(2 * t * Math.PI/180)
        ctx.beginPath()
        ctx.ellipse(x, y, a, b, phi, 0, Math.PI * 2)
        ctx.stroke()

        let phi1 = Math.PI / 12 - Math.sin(2 * t * Math.PI/180)
        let [a1, b1] = [a - 100, b - 25]
        ctx.beginPath()
        ctx.ellipse(x, y, a1, b1, phi1, 0, Math.PI * 2)
        ctx.stroke()

        let phi2 = Math.PI / 4 - Math.sin(2 * t * Math.PI/180)
        let [a2, b2] = [a - 200, b - 50]
        ctx.beginPath()
        ctx.ellipse(x, y, a2, b2, phi2, 0, Math.PI * 2)
        ctx.stroke()

        let size3 = (0.5 + 0.5 * Math.sin(Math.PI * (angle + 30) / 180)) * 20
        let [x3, y3] = [x + a2 * Math.cos((angle + 30) * Math.PI / 180) * Math.cos(phi2) - b2 * Math.sin((angle + 30) * Math.PI / 180) * Math.sin(phi2),
        y + a2 * Math.cos((angle + 30) * Math.PI / 180) * Math.sin(phi2) + b2 * Math.sin((angle + 30) * Math.PI / 180) * Math.cos(phi2)]
        ctx.beginPath()
        ...
        let [x2, y2] = [x + a1 * Math.cos((angle + 15) * Math.PI / 180) * Math.cos(phi1) - b1 * Math.sin((angle + 15) * Math.PI / 180) * Math.sin(phi1),
        y + a1 * Math.cos((angle + 15) * Math.PI / 180) * Math.sin(phi1) + b1 * Math.sin((angle + 15) * Math.PI / 180) * Math.cos(phi1)]
        ctx.beginPath()
        ctx.arc(x2, y2, 15 + size2, 0, Math.PI * 2)
        ctx.fill()

        let size = (0.5 + 0.5 * Math.sin(Math.PI * angle / 180)) * 20
        let [x1, y1] = [x + a * Math.cos(angle * Math.PI / 180) * Math.cos(phi) - b * Math.sin(angle * Math.PI / 180) * Math.sin(phi),
        y + a * Math.cos(angle * Math.PI / 180) * Math.sin(phi) + b * Math.sin(angle * Math.PI / 180) * Math.cos(phi)]
        ctx.beginPath()
